...
def setDemande():
	...
	while cpt != len(demande) or len(demande) > 10:		#Tant que cpt ne vaut pas la taille de la chaine demande ou que la longueur de la chaine saisie est > 10...
		...
		if cpt != len(demande) or len(demande) > 10:	#	Si cpt est différent de la taille de demande ou que la longueur de la chaine saisie est > 10...
...
#value(int):	Valeur à convertir en degrés.
#scale(int):	Échelle de l'animation.
#pos(float):	Position en ordonnée.
...
def Animate(frames,xmin,ymin,xmax,ymax,size,delay,it):
	print('\n' + str(len(frames)) + " Images, temps estimé pour la génération " + str(round(len(frames)/100,0)) + " minute(s).")	#Affichage du temps estimé pour la génération de l'animation.
